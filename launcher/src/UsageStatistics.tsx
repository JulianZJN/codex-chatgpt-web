...
export function UsageStatisticsSection({ api, language }: { api: LauncherApi; language: Language }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setData(null);
    // The promise boundary also catches a missing IPC method in an older launcher.
    Promise.resolve().then(() => api.getUsageStatistics({ days })).then((result) => {
...
export function UsageStatisticsView({ data, days, language, loading, onDaysChange, onRefresh }: {
  ...
  return <section className="usage-section" aria-labelledby={`${id}-title`} aria-busy={loading}>
    ...
    {ready && <>
      ...
      <div className="usage-chart">
        <svg viewBox="0 0 600 218" role="group" aria-label={`${copy.daily}: ${number(total)} ${copy.total}`}>
          ...
          {data.days.map((day, index) => {
            ...
            return <g key={day.day} tabIndex={0} role="img" aria-label={description} className="usage-day"
              onFocus={() => setActiveDay(day.day)} onBlur={() => setActiveDay(null)} onMouseEnter={() => setActiveDay(day.day)} onMouseLeave={() => setActiveDay(null)}>
      ...
      <ul className="usage-legend" aria-label={copy.tier}>{tiers.map((tier) => <li key={tier}><i style={{ background: colors[tier] }} /><span>{labels[tier]}</span><b>{number(count(data.days.map((day) => day.tiers[tier])))}</b></li>)}</ul>
      ...
      <div className="usage-table-scroll"><table className="usage-table"><thead><tr>{[copy.version, copy.source, copy.accepted, copy.completed, copy.error].map((label) => <th scope="col" key={label}>{label}</th>)}</tr></thead><tbody>
        {lifetimeRows.map((row) => <tr key={`${row.version}-${row.source}`}><th scope="row">{row.version === "unknown" ? copy.proUnknown : `GPT-${row.version} Pro`}</th><td>{row.source ? sourceLabel(row.source) : "—"}</td><td>{number(row.accepted)}</td><td>{number(row.completed)}</td><td>{number(row.error)}</td></tr>)}
      </tbody></table></div>
      <details className="usage-details"><summary>{copy.details}</summary><div className="usage-table-scroll"><table className="usage-table"><thead><tr>{[copy.date, copy.tier, copy.accepted, copy.completed, copy.error].map((label) => <th scope="col" key={label}>{label}</th>)}</tr></thead><tbody>
        {data.days.flatMap((day) => {
          const active = tiers.filter((tier) => Object.values(day.tiers[tier]).some((value) => value > 0));
          return active.length ? active.map((tier) => <tr key={`${day.day}-${tier}`}><th scope="row">{formatDay(day.day, language)}</th><td>{labels[tier]}</td><td>{number(day.tiers[tier].accepted)}</td><td>{number(day.tiers[tier].completed)}</td><td>{number(day.tiers[tier].error)}</td></tr>) : [<tr key={day.day}><th scope="row">{formatDay(day.day, language)}</th><td>{copy.none}</td><td>0</td><td>0</td><td>0</td></tr>];
        })}
    ...
    <div className="usage-footnotes"><p>{copy.boundary}</p>
      ...
      <details><summary>{copy.accepted} / {copy.completed}</summary><p>{copy.semantics}</p><p>{copy.privacy}</p></details>
    </div>
  </section>;
}
