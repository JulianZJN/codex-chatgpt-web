...
- **ローカル使用統計。** 設定画面で段階別の 7/30 日チャート、GPT-5.6 Pro の当日回数、GPT-6 Pro の週間回数、Pro の累計表を確認できます。ローカルで送信を確認したメッセージ数であり、公式の残り利用枠ではありません。[集計方法とプライバシー](docs/local-usage-statistics.md)を参照してください。
